...
class ODEblock(nn.Module):
  def __init__(self, odefunc, opt,    device, t):#data,
    super(ODEblock, self).__init__()
    self.opt = opt
    self.t = t
    

    self.odefunc = odefunc(opt['hidden_dim'], opt['hidden_dim'], opt,   device) #data,
...
class ODEFunc(MessagePassing):
  ...
  def __init__(self, opt, device):  #data,
...
class BaseGNN(MessagePassing):
  def __init__(self, opt, num_features, device=torch.device('cpu')):
    ...
    self.opt = opt
    self.T = opt['time']
    self.num_classes =opt['num_classes']  #dataset.num_classes 
