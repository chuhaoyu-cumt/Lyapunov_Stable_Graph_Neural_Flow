...
class ODEFuncTransformerAtt(ODEFunc):
  ...
  def __init__(self, in_features, out_features, opt, device):
    ...
    # else:
    #   self.edge_index, self.edge_weight = data.edge_index.to(device), data.edge_attr.to(device)
    self.multihead_att_layer = SpGraphTransAttentionLayer(in_features, out_features, opt,
                                                          device, edge_weights=self.edge_weight).to(device) ##实现基于热扩散方程的图神经网络
    input_shape = in_features
    control_size = 0
    lyapunov_lr = 3e-4
    lyapunov_eps = 1e-3
    layer_sizes = [in_features,in_features]
    ...
    self.proj_lyapunov = nn.Linear(in_features,in_features).to(device)
    #self.norm_lyapunov = nn.BatchNorm1d(in_features).to(device)
    

  def multiply_attention(self, x, attention, v=None):  ##多头注意力机制
  ...
  def forward(self, t, x):  # t is needed when called by the integrator  x=u 节点特征
    ...
    f = alpha * (ax - x) ##公式27  debug输出参数的shape    f.shape:[2485,64]
    
    ##TODO:f基础动力系统 投影至liyapunofu f_hat,
    
      # ##定义一个投影层，linear
      # f_proj = self.proj_lyapunov(f)
    ...
    #     f_lyapunov_i = self._lyapunov_function(f[i].unsqueeze(0))
   ...
   # f_lyapunov = self._lyapunov_function(f)#.squeeze()
   # print(f_lyapunov.shape)
    ...
    #return f_lyapunov.squeeze(1)
    return f #f_lyapunov.squeeze(1) #f

  def __repr__(self):
    return self.__class__.__name__ + ' (' + str(self.in_features) + ' -> ' + str(self.out_features) + ')'


class SpGraphTransAttentionLayer(nn.Module):  #稀疏的图注意力网络层
  """
  ...
  def forward(self, x, edge):
    ...
    if self.opt['square_plus']:
      ...
      attention = softmax(prods, edge[self.opt['attention_norm_idx']].to(self.device ))
    return attention, (v, prods) ##attention：注意力权值。 
...
if __name__ == '__main__':
  ...
  opt = {'dataset': 'Cora', 'self_loop_weight': 1, 'leaky_relu_slope': 0.2, 'heads': 2, 'K': 10,
         ...
         'alpha_dim': 'sc', 'beta_dim': 'sc', 'max_nfe': 1000, 'mix_features': False, 'random_splits':False,'attention_type':'scaled_dot',
         'reweight_attention':False, 'square_plus':False, 'attention_norm_idx':0, 'no_alpha_sigmoid': False,
         }
  dataset = get_dataset(opt, '../data', False)#.to(device)
  t = 1
  func = ODEFuncTransformerAtt(dataset.data.num_features, 6, opt, dataset.data, device) #dataset.data.num_features:1433 
  input = dataset.data.x
  out = func(t, input.to(device)) ##dataset.data.x.shape:[2708,1433]
