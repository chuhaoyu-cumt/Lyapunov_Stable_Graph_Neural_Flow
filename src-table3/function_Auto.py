...
class ODEFuncAuto(ODEFunc):

  def __init__(self, in_features, out_features, opt, data, device):
    super(ODEFuncAuto, self).__init__(opt, data, device)
  ...
  def forward(self, t, x):  # t is needed when called by the integrator
    ...
    f = a_0 + torch.mul(a_1, ax) + torch.mul(a_2, x) + torch.mul(a_3, x_2) + torch.mul(a_4, x_3) + torch.mul(a_5, x_4) + torch.mul(a_6, x_5), torch.mul(a_7, x_6) + torch.mul(a_8, x_7)
