...
def test_defend_robo(args,data,ckpt_path,j):
    ...
    model = GNN(opt, features.shape[1], args.device)
...
def test_defend(args,data,ckpt_path):
    features = data.features
    labels = data.labels
    adj = data.adj
    ...
    #   # # #  ##保存模型
    # save_name = f"model_{i}_{best_epoch}_{best_time}_{args.ptb_rate}.pth"
    # save_path = os.path.join(args.out_path, save_name)
...
def main(args,ckpt_path,j):
    args.device = torch.device('cuda', args.gpu)
    set_seed(args.seed)

    # Load dataset
    data = Dataset(root='/data/Cora/', name='cora', setting='prognn')
    ...
    _,_, test_acc_clean = test_defend(args,data, ckpt_path)
    ...
    _,_, test_acc_adv = test_defend_robo(args,data,ckpt_path,j)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=800,
                        help='Number of epochs to train.') #500
    ...
    parser.add_argument('--decay', type=float, default=0.01, help='Weight decay for optimization')#5e-4
    ...
    parser.add_argument('--block', type=str, default='att_frac', help='constantfrac')
    parser.add_argument('--function', type=str, default='transgrand', help='transgrand, transformer, belgrand,belgrand')
    ...
    # ODE args
    parser.add_argument('--time', type=float, default=4, help='End time of ODE integrator.')
    ...
    parser.add_argument('--alpha_ode', type=float, default=0.85, help='alpha_ode')
    ...
    parser.add_argument('--out_path', type=str, default='/stage-first-basegnn+ori-transgrand/')
    ...
    for ptb_rate in [0.05,0.1,0.15,0.2,0.25]:
        ...
        result_info['test_adv_list'] = test_adv
        with open(filename_log, 'a') as f:
