...
def tensor_to_adj(adj_tensor):
    ...
    elif isinstance(adj_tensor, torch.Tensor) and adj_tensor.layout == torch.sparse_coo:
    # 转换为 SparseTensor
        device = adj_tensor.device
        adj_tensor = SparseTensor.from_torch_sparse_coo_tensor(adj_tensor)
        adj_tensor = adj_tensor.to(device)
    

        if  type(adj_tensor) is torch_sparse.tensor.SparseTensor:
       ...
       # PyG graph
        ##ori
        #adj = csr_matrix((torch.ones(M).cpu().numpy(), (adj_tensor[0].cpu().numpy(),adj_tensor[1].cpu().numpy())), (M,M),np.int64) 


        
        ##cxt
        M = adj_tensor.max().item() + 1
        adj = csr_matrix((torch.ones(len(adj_tensor[0])).cpu().numpy(), (adj_tensor[0].cpu().numpy(),adj_tensor[1].cpu().numpy())), (M,M),np.int64) #cxt修改
