...
def gcn_norm(adj_t, order=-0.5, add_self_loops=True):
    ##添加类型判断
     # 如果 adj_t 是 PyTorch 的稀疏张量，则转换为 PyG SparseTensor
    if isinstance(adj_t, torch.Tensor) and adj_t.is_sparse:
        indices = adj_t._indices()
        values = adj_t._values()
        size = adj_t.shape
        adj_t = SparseTensor(row=indices[0], col=indices[1], value=values, sparse_sizes=size)
...
# gia feature upd
def gia_update_features(attacker, model, adj_attack, features, features_attack, origin_labels, target_idx, homophily=None, hinge=False,device='cpu'):
    attacker.early_stop.reset()
    if hasattr(attacker, 'disguise_coe'):
        disguise_coe = attacker.disguise_coe
    else:
    ...
    feat_lim_min, feat_lim_max =  torch.tensor(attacker.feat_lim_min).to(device),  torch.tensor(attacker.feat_lim_max).to(device)
    n_total = features.shape[0]
    model.eval()
    ...
    for i in range(n_epoch):
        ...
        # print("feature attack device: ",features_attack.device )
