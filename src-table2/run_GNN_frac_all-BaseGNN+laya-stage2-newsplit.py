...
from utils import ROOT_DIR, target_select, prune_graph, inductive_split, get_index_induc,feat_normalize
import sys
import json
...
def eval_robustness(model, features, adj, target_idx, labels, device, args, run):
    ...
    if args.eval_robo_blk:
        graph_path = os.path.join(args.save_attack,args.dataset.lower())+f"_{args.eval_attack}"
        if args.eval_target:
            graph_path += "_target"
...
def train(model, optimizer, data, train_idx):
  ...
  out = model(feat, data.adj_t) #data.edge_index
...
def test_OGB(model, data, opt):
  ...
  if opt['dataset'] == 'ogbn-arxiv':
    ...
    out = model(feat, data.adj_t).log_softmax(dim=-1)
    y_pred = out.argmax(dim=-1, keepdim=True)
...
def test(model, data,  opt=None):  # opt required for runtime polymorphism
  ...
  if model.opt['use_labels']:
    feat = add_labels(feat, data.y, data.train_mask, model.num_classes, model.device)
  logits, accs = model(feat, data.adj_t), []  ##
...
def main(opt,split):
  ...
  dataset = Planetoid('/amax/home/Frond_laya_remix1_Cora/data/Cora/', 'Cora', transform=transform)
  data = dataset[0]
  data.adj_t = data.adj_t.to_symmetric()
  num_classes = dataset.num_classes

  data.x = feat_normalize(data.x,norm=args.hidden_dim)
  ...
  if args.inductive: ##here
        ...
        adj_train, adj_val, adj_test = inductive_split(data.adj_t, split_idx)#data_1.adj_t  #data.edge_index
...
if __name__ == '__main__':
  ...
  filename = str(args.out_path) + str(args.dataset) + str(args.method) + str(args.function) + str(args.block) + str(
  ...
  print("robust_test acc: ", robust_best)
   ...
   ##选择top-10个best结果
  top10_best = sorted(best, reverse=True)[:10]

  top10_robust_best = sorted(robust_best, reverse=True)[:10]
  ...
  with open(filename, 'a') as f:
    ...
    f.write('Mean robust_test accuracy:')
    f.write(str(np.mean(np.array(robust_best) * 100)))
    f.write('std: ')
    f.write(str(np.std(np.array(robust_best) * 100)))
    f.write("\n")
