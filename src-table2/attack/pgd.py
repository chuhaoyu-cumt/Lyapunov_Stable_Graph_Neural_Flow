...
class PGD(object):
    ...
    def attack(self, model, adj, features, target_idx, labels=None):
        ...
        if labels == None:
            pred_orig = model(features,adj)#, adj
    ...
    def injection(self, adj, n_inject, n_node, target_idx):
        ...
        # print("adj:", adj.shape)
        # print("adj:", adj)
        # if adj is not a cipy.sparse.csr.csr_matrix, convert it to csr_matrix
        # if not isinstance(adj, sp.csr_matrix):
        #     print("adj is not a cipy.sparse.csr.csr_matrix, convert it to csr_matrix")
        ...
        add1 = sp.csr_matrix((n_inject, n_node)).tocoo()
        add2 = sp.csr_matrix((n_node + n_inject, n_inject)).tocoo()
    ...
    def update_features(self, model, adj_attack, features, features_attack, origin_labels, target_idx):
        ...
        #ori
        #feat_lim_min, feat_lim_max = self.feat_lim_min.to(self.device), self.feat_lim_max.to(self.device)
        feat_lim_min = torch.tensor(self.feat_lim_min, dtype=torch.float32).to(self.device)
        feat_lim_max = torch.tensor(self.feat_lim_max, dtype=torch.float32).to(self.device)
        ...
        for i in range(n_epoch):
            ...
            features_concat = torch.cat((features, features_attack), dim=0)  #注入被攻击的节点
            ...
            # print("features_attack device: ", features_attack.device)
            # print("feat_lim_min device: ", feat_lim_min.device)
            # print("feat_lim_max device: ", feat_lim_max.device)
    ...
    def adv_attack(self, model, adj, features, target_idx, optimizer, labels=None ):
        model.to(self.device)
        model.eval()
        n_total, n_feat = features.shape
        ...
        adj_attack = self.adv_injection(adj=utils.tensor_to_adj(adj),
                                    n_inject=self.n_inject_max,
                                    n_node=n_total,
                                    target_idx=target_idx)
        adj_attack = utils.adj_to_tensor(adj_attack).to(target_idx.device)
        ...
        features_attack = self.adv_update_features(model=model,
                                               adj_attack=adj_attack,
                                               features=features,
                                               features_attack=features_attack,
                                               origin_labels=origin_labels,
    ...
    def adv_injection(self, adj, n_inject, n_node, target_idx):
        r"""
    ...
    def adv_update_features(self, model, adj_attack, features, features_attack, origin_labels, target_idx,optimizer):
        r"""
        ...
        for name, param in model.named_parameters():
            if name.startswith('odeblock._lyapunov_function'):
