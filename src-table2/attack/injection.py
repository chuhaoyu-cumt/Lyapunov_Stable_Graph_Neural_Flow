...
def tdgia_injection(adj, n_inject, n_edge_max, origin_labels, current_pred, 
    ...
    for i in range(n_inject):
        for j in range(n_connect):
            ...
            # print("injecting", i, "th node, ", j, "th edge")
            # print("label", small_id, "pos", pos[small_id], "len", len(labelil[small_id]))
            if pos[small_id] >= len(labelil[small_id]):
                print("skip this edge because of no more nodes")
                continue
            tu = labelil[small_id][int(pos[small_id])]
            pos[small_id] = pos[small_id] +1
            x = n_current + i
            y = target_idx[tu]
            new_edges_x.extend([x, y])
            new_edges_y.extend([y, x])
    ...
    adj_attack = sp.vstack([adj, add1]).tocoo()
    adj_attack = sp.hstack([adj_attack, add2]).tocoo()
    adj_attack.row = np.hstack([adj_attack.row, new_edges_x])
    adj_attack.col = np.hstack([adj_attack.col, new_edges_y])
    adj_attack.data = np.hstack([adj_attack.data, new_data])
    adj_attack = utils.adj_to_tensor(adj_attack).to(device)
...
#     adj_attack = torch.sparse_coo_tensor(
#     torch.stack([new_row, new_col]), 
#     torch.cat((z, vals, vals)), 
#     requires_grad=True
# )
    #adj_attack.requires_grad_(True)
